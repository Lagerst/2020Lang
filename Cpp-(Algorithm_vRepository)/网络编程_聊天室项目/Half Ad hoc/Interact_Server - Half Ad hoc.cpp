...
pair<string,int> LoginUser(string s,string ip,int port, string * usr){
    ...
    users[string(itemName->valuestring)]=std::make_pair(ip,port);

    if (strcmp(itemOrder->valuestring,"getmap")){
...
void process(int sockfd){
    ...
    while(1){
        ...
        redirect(std::make_pair(str,port),make_Address(target,usr).c_str());
    }
}