  ...
  while (1) {
    ...
    for (int i = 0; i < iFdCnt; i++) {
      if (aoEvents[i].data.fd == listenfd) {
        ...
        int iAcceptFd =
            accept(listenfd, (struct sockaddr *)&oClientAddr, &iAddrLen);

        printf("recv connection from %s:%d\n", inet_ntoa(oClientAddr.sin_addr),
               ntohs(oClientAddr.sin_port));
        ...
        if (iSendLen < 0) {
          ...
          if (epoll_ctl(iEpollFd, EPOLL_CTL_DEL, iCurFd, NULL) < 0) {
            // cerr << "fail to del fd from epoll, err: " << strerror(errno) <<
            // endl;
          }
          close(iCurFd);
          break;
