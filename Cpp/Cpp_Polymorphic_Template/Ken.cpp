...
public:
  ...
  virtual ~Graph() {}
  static size_t get_random_num(int modules) { return rand() % modules + 1; }
...
public:
  ...
  string getname() { return string("parallelogram"); }
  string getpara() {
    return string("width=") + to_string(width) + string(", height=") +
           to_string(height);
  }
};
...
public:
  rectangle() {}
  string getname() { return string("rectangle"); }
  string getpara() {
    return string("width=") + to_string(width) + string(", height=") +
           to_string(height);
  }
...
public:
  ...
  string getname() { return string("square"); }
  string getpara() { return string("width=") + to_string(width); }
};

class diamond : public parallelogram {
public:
  diamond() { area = area / 2; }
  string getname() { return string("diamond"); }
  string getpara() {
    return string("diagnal1=") + to_string(width) + string(", diagnal2=") +
           to_string(height);
  }
};
...
public:
  ...
  string getname() { return string("trapezoid"); }
  string getpara() {
    return string("width1=") + to_string(width1) + string(", width2=") +
           to_string(width2) + string(", height=") + to_string(height);
  }
};
...
public:
  ...
  template <typename G> void do_record(G &p) {
    auto *temp = new node;
    temp->graph = &p;
    temp->next = nullptr;
    now->next = temp;
...
int main() {
  ...
  // Graph initlist[5]={new parallelogram(),new rectangle(),new diamond(),new
  // diamond(),new square()}; Graph&
  // p=static_cast<typeid(initlist[Graph::get_random_num(5)])> (new Graph());
  // //wrong
  ...
  for (auto it = l->begin(); it != nullptr; ++it) {
    cout << (*it).graph->getname() << ": " << (*it).graph->getpara()
         << string(", answer=") << (*it).graph->getrecord()
         << ((*it).graph->getrecord() == (*it).graph->getarea()
                 ? (++correct, string(" V"))
                 : (++wrong, string(" X")))
         << endl;
  }
  ...
  cout << endl
       << "Total: " << correct + wrong << ", Correct: " << correct << endl;
